...
export const EnergyProvider: React.FC<EnergyProviderProps> = ({ children }) => {
  const [remainedEnergy, setRemainedEnergy] = useState(() => {
    const storedEnergy = (typeof window !== "undefined" && localStorage) ? localStorage.getItem("remainedEnergy") : "0";
  ...
  return (
    <EnergyContext.Provider
      value={{ remainedEnergy, setRemainedEnergy, startRecovery, stopRecovery }}
    >
      {children}
    </EnergyContext.Provider>
  );
